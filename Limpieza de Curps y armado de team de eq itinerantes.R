...
base_Carlos <- df %>% 
  filter(turno %in% c("Equipo itinerante", "EQUIPO ITINERANTE")) %>% 
  transmute(
    estado,
    clues_ancla = clues,
...
curps_limpio <- curps_limpias %>% 
  clean_names() %>% 
  transmute(
    curp,
    nombre = paste(ape_pat, ape_mat, nombres)
  ) %>% 
...
# Base Carlos con cluster y duplicados ------------------------------------

base_Carlos_1 <- base_Carlos %>% 
  left_join(
    base_alex %>% distinct(clues_imb, .keep_all = TRUE),
    by = c("clues_ancla" = "clues_imb")
  ) %>%
...
duplicados_link_feo <- base_Carlos_3 %>% 
  left_join(
    links_buenos %>% 
      mutate(link_bueno = TRUE),
    by = c("duplicados", "enlace_a_carpeta")
...
observaciones_eliminadas_filtros <- base_Carlos %>% 
  anti_join(
    base_corregida %>% 
